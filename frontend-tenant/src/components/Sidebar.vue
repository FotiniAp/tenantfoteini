        ...
        <div :class="isOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
            ...
            <div class="flex items-center justify-center mt-8">
                <div class="flex items-center">
                    ...
                    <span class="mx-2 text-2xl font-semibold text-white">Tenant</span>
                </div>
            </div>

            <nav class="mt-10">
                <router-link class="flex items-center px-6 py-2 mt-4 duration-200 border-l-4"
                    :class="[$route.name === 'home' ? activeClass : inactiveClass]" :to="{ name: 'home' }">
                    <span class="mx-4">Home</span>
                </router-link>
                <router-link class="flex items-center px-6 py-2 mt-4 duration-200 border-l-4" v-if="role == 'tenant_admin'"
                    :class="[$route.name === 'users' ? activeClass : inactiveClass]" :to="{ name: 'users' }">
                    <span class="mx-4">Users</span>
                </router-link>
                <div class="flex items-center px-6 py-2 mt-4 duration-200 border-l-4" @click="logout"
